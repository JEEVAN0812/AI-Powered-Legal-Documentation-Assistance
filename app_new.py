...
import json 
from bert_dataset import ask_question_and_get_answer
...
@app.route("/api3/message", methods=["POST"])
def api_message3():
    message = request.json["message"]
    print("Received message: ", message)
    file_path = "static/pdf/legal.txt"
    question, context, answer = ask_question_and_get_answer(file_path, message)
